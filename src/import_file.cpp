...
void ImportFile::Draw(Context context) {
    ...
    if (this->open) {
        ...
        GuiLabel((Rectangle){this->offset.x + 10, this->offset.y + 30, 65, 20},
                 "Import file:");
        GuiLabel((Rectangle){this->offset.x + 85, this->offset.y + 30, 75, 20},
                 fileName);
        GuiLabel((Rectangle){this->offset.x + 10, this->offset.y + 50, 65, 20},
                 "File size:");
        GuiLabel((Rectangle){this->offset.x + 85, this->offset.y + 50, 75, 20},
                 TextFormat("%i bytes", dataSize));
        GuiGroupBox(
            (Rectangle){this->offset.x + 10, this->offset.y + 85, 180, 80},
            "Resolution");
        GuiLabel((Rectangle){this->offset.x + 20, this->offset.y + 100, 33, 25},
                 "Width:");
        if (GuiValueBox(
                (Rectangle){this->offset.x + 60, this->offset.y + 100, 80, 25},
                NULL, &this->widthValue, 0, 8192, this->widthEditMode))
            this->widthEditMode = !this->widthEditMode;
        GuiLabel(
            (Rectangle){this->offset.x + 145, this->offset.y + 100, 30, 25},
            "pixels");
        GuiLabel((Rectangle){this->offset.x + 20, this->offset.y + 130, 33, 25},
                 "Height:");
        if (GuiValueBox(
                (Rectangle){this->offset.x + 60, this->offset.y + 130, 80, 25},
                NULL, &this->heightValue, 0, 8192, this->heightEditMode))
            this->heightEditMode = !this->heightEditMode;
        GuiLabel(
            (Rectangle){this->offset.x + 145, this->offset.y + 130, 30, 25},
            "pixels");
        GuiGroupBox(
            (Rectangle){this->offset.x + 10, this->offset.y + 180, 180, 160},
            "Pixel Format");
        GuiComboBox(
            (Rectangle){this->offset.x + 20, this->offset.y + 195, 160, 25},
            TextJoin(context.pixelFormatTextList, 8, ";"), &pixelFormatActive);
        GuiLine((Rectangle){this->offset.x + 20, this->offset.y + 220, 160, 20},
                NULL);

        if (pixelFormatActive != 0)
            GuiDisable();
        GuiLabel((Rectangle){this->offset.x + 20, this->offset.y + 235, 50, 20},
                 "Channels:");
        GuiToggleGroup(
            (Rectangle){this->offset.x + 20, this->offset.y + 255, 156 / 4, 25},
            TextJoin(context.channelsTextList, 4, ";"), &channelsActive);
        GuiLabel((Rectangle){this->offset.x + 20, this->offset.y + 285, 50, 20},
                 "Bit Depth:");
        GuiToggleGroup(
            (Rectangle){this->offset.x + 20, this->offset.y + 305, 160 / 3, 25},
            TextJoin(context.bitDepthTextList, 3, ";"), &bitDepthActive);
        GuiEnable();

        GuiGroupBox(
            (Rectangle){this->offset.x + 10, this->offset.y + 355, 180, 50},
            "Header");
        GuiLabel((Rectangle){this->offset.x + 25, this->offset.y + 370, 27, 25},
                 "Size:");
        if (GuiValueBox(
    ...
    if (btnLoadPressed) {
        ...
        if ((this->widthValue != 0) && (this->heightValue != 0)) {
            ...
            if (pixelFormatActive == 0) {
                int channels = atoi(context.channelsTextList[channelsActive]);
            ...
            if (format != -1) {
                ...
                Project pr = Project(this->widthValue, this->heightValue,
                                     this->fileName, this->fileNamePath, "\0");
